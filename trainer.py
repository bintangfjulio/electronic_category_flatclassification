...
if __name__ == "__main__":
    ...
    num_classes = len(dataset['category'].drop_duplicates().values.tolist())
    
    module = Preprocessor(batch_size=32, dataset=dataset) 
    model = IndoBERT_CNN(lr=2e-5, num_classes=num_classes)

    checkpoint_callback = ModelCheckpoint(dirpath='./checkpoints/indobertcnn_results', monitor='val_loss')
    logger = TensorBoardLogger("logs", name="indobertcnn_results")
    ...
    trainer = pl.Trainer(
        accelerator='gpu',
        max_epochs=100,
        default_root_dir="./checkpoints/indobertcnn_results",
