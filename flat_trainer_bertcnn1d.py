...
from models.flat_bertcnn1d import Flat_BERTCNN1D
...
if __name__ == "__main__":
    ...
    model = Flat_BERTCNN1D(lr=2e-5, num_classes=len(target_names))

    checkpoint_callback = ModelCheckpoint(dirpath='./checkpoints/flat_bertcnn1d_results', monitor='val_loss')
    logger = TensorBoardLogger("logs", name="flat_bertcnn1d_results")
    early_stop_callback = EarlyStopping(monitor='val_loss', min_delta=0.00, check_on_train_epoch_end=1, patience=3)

    trainer = pl.Trainer(
        accelerator='gpu',
        max_epochs=30,
        default_root_dir="./checkpoints/flat_bertcnn1d_results",
